...
public class RandomHistoryRepository {
  ...
  public RandomHistory save(RandomHistory history) {
    jdbcClient.sql("""
        INSERT INTO random_histories (id, user_id, restaurant_id, area, genre, budget_min, budget_max, created_at)
        VALUES (:id, :userId, :restaurantId, :area, :genre, :budgetMin, :budgetMax, :createdAt)
        """)
        .param("id", history.id())
        ...
        .param("createdAt", history.createdAt())
        .update();
    return history;
  }
  ...
  public List<RandomHistory> findByUserId(String userId) {
    return jdbcClient.sql("""
        SELECT id, user_id, restaurant_id, area, genre, budget_min, budget_max, created_at
